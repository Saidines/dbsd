...
int BlockBuffer::loadBlockAndGetBufferPtr(unsigned char **buffPtr) {
    ...
    if (bufferNum == E_BLOCKNOTINBUFFER) {
        bufferNum = StaticBuffer::getFreeBuffer(this->blockNum); // get a free buffer using StaticBuffer.getFreeBuffer()

        if (bufferNum == E_OUTOFBOUND) {
        return E_OUTOFBOUND; // if no free buffer is available, return E_OUTOFBOUND
        }

        Disk::readBlock(StaticBuffer::blocks[bufferNum], this->blockNum); // read the block from disk to buffer
    }
